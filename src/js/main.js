//lesson-13
const entryCountry = document.querySelector('.discovery-search__nw');
const bottomSearch = document.querySelector('.discovery-search');

entryCountry.addEventListener('click', (event) => {
...
bottomSearch.addEventListener('click', async (event) => {
  document.querySelector('.available').style.display = 'flex';
  event.preventDefault(); //не отправит действия брайзера
  const inputCountry = document
    .querySelector('.input-country')
    .value.toLowerCase();
  console.log(inputCountry);
  await fetch(
    `https://if-student-api.onrender.com/api/hotels?search=${inputCountry}`,
  ).then((response) => {
    if (!response.ok) {
